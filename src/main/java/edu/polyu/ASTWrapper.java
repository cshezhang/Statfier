...
public class ASTWrapper {
    ...
    public ASTWrapper(String filePath) {
        this.depth = 0;
    ...
    public void printBasicInfo() {
        ...
        for(TypeDeclaration td : this.types) {
            System.out.println("----------Type(Class) Name: " + td.getName() + "----------");
...
//            System.out.println("No violations in this file, it needs more checks!");
        ...
        for(Statement statement : candidateStatements) {
            if(statement instanceof VariableDeclarationStatement) {
                rightExpression = ((VariableDeclarationFragment) ((VariableDeclarationStatement) statement).fragments().get(0)).getInitializer();
            }
            if(statement instanceof ExpressionStatement && ((ExpressionStatement) statement).getExpression() instanceof Assignment) {
                rightExpression = ((Assignment) ((ExpressionStatement) statement).getExpression()).getRightHandSide();
            }
        }
        if(rightExpression != null) {
            ...
            if(rightExpression instanceof ClassInstanceCreation) {
                ...
                // Improvement: We may consider arguments of MethodInvocation's arguments, e.g., str.get(str1.get());
            ...
            List<Statement> subStatements = getSubStatements(getDirectMethodOfStatement(candidateStatements.get(0)));
