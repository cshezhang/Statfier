...
public class UnboundIdLdap {
  ...
  static boolean authenticate(String username, String password) throws LDAPException {
    ...
    SearchResult sr =
        ldap.search("dc=People,dc=example,dc=com", SearchScope.SUB, "(uid=" + username + ")");
    if (sr.getEntryCount() == 0) return false;
    ...
    try {
      ...
      if (e.getResultCode() == ResultCode.INVALID_CREDENTIALS) return false;
