...
public class SpringLdap {
  ...
  public void queryVulnerableToInjection(
    ...
    template.search(
        ...
        "dn=1",
        searchControls,
        new CountNameClassPairCallbackHandler(),
        dirContextProcessor);
    template.search(
        jndiInjectMe,
        "dn=1",
    ...
    template.search(
        jndiInjectMe, "dn=1", searchControls, new LdapEntryIdentificationContextMapper());
    template.search(jndiInjectMe, "dn=1", searchControls, new DefaultIncrementalAttributesMapper());
  }

  public void safeQuery(
      LdapTemplate template, SearchControls searchControls, DirContextProcessor dirContextProcessor)
      throws NamingException {
    ...
    template.search(
        ...
        new DefaultIncrementalAttributesMapper());
    template.search(
        safeQuery, "dn=1", SearchControls.OBJECT_SCOPE, new DefaultIncrementalAttributesMapper());
    ...
    template.search(
        ...
        new LdapEntryIdentificationContextMapper());
    template.search(
        safeQuery, "dn=1", SearchControls.OBJECT_SCOPE, new LdapEntryIdentificationContextMapper());
