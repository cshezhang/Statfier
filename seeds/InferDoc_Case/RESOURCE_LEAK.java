...
public class RESOURCE_LEAK {
  ...
  // and you should use the standard idiom for the most part, when you don't want to return the
  // resource to the surrounding context.

  // Sometimes people just leave out close(), and that is a bug, but more typically exceptional
  // paths are the root of the problem, as in
  ...
  // Here, if there is an exception in the call to fis.close() execution will skip past fos.close();
  // a leak.
  ...
  public static void foo6() throws IOException {
    try (FileInputStream fis = new FileInputStream(new File("whatever.txt"));
        FileOutputStream fos = new FileOutputStream(new File("everwhat.txt"))) {
      fos.write(fis.read());
    }
  }
