...
class DateFormatWeekYearCheck {
  ...
  void useSimpleDateFormat() throws ParseException {
    ...
    result =
        new SimpleDateFormat("YYYY-ww")
            .format(
                date); // compliant, 'Week year' is used along with 'Week of year'. result =
                       // '2016-01'
    result =
  ...
  public void useDateTimeFormatter() {
    ...
    DateTimeFormatter.ofPattern(
        "  Y/MM/dd",
        Locale
            .ENGLISH); // Noncompliant [[sc=33;ec=44]] {{Make sure that week Year "Y" is expected
                       // here instead of Year "y".}}
    DateTimeFormatter.ofPattern(
        "YY",
        Locale
            .ENGLISH); // Noncompliant [[sc=33;ec=37]] {{Make sure that week Year "YY" is expected
                       // here instead of Year "yy".}}
    DateTimeFormatter.ofPattern(
        "Y",
        Locale
            .ENGLISH); // Noncompliant [[sc=33;ec=36]] {{Make sure that week Year "Y" is expected
                       // here instead of Year "y".}}
    DateTimeFormatter.ofPattern(
  ...
  class NonCompliantChildOfSimpleDateFormat extends SimpleDateFormat {
    public NonCompliantChildOfSimpleDateFormat() {
      super(
          "YYYY"); // Noncompliant {{Make sure that week Year "YYYY" is expected here instead of
                   // Year "yyyy".}}
    }
  }
}
