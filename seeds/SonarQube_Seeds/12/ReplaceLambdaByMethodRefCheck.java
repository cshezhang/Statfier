  ...
  void nullChecks(List<String> strings, String s2) {
    strings.stream()
        .filter(
            s ->
                s
                    != null); // Noncompliant {{Replace this lambda with method reference
                              // 'Objects::nonNull'.}}
    strings.stream()
        .filter(
            s -> {
    ...
    strings.stream()
        .filter(
            s ->
                null
                    == s); // Noncompliant {{Replace this lambda with method reference
                           // 'Objects::isNull'.}}
    strings.stream()
        .filter(
            s ->
  ...
  void testInstanceOf(Object param) {
    barbar(
        (o) ->
            o
                instanceof
                String); // Noncompliant {{Replace this lambda with method reference
    ...
    barbar4(
        (o) ->
            o
                instanceof
                TestA); // Noncompliant {{Replace this lambda with method reference
    ...
    barbar3(
        (o) ->
            o
                instanceof
                List); // Noncompliant {{Replace this lambda with method reference
    ...
    barbar5(
        (o) ->
            o
                instanceof
                TestA
                    []); // Noncompliant {{Replace this lambda with method reference
                         // 'TestA[].class::isInstance'.}}
    ...
    barbar9(
        (o) ->
            o
                instanceof
                Character); // Noncompliant {{Replace this lambda with method reference
  ...
  void testCasts(Object param) {
    ...
    "abc"
        ...
        .mapToObj(
            i ->
                (char)
                    i); // Compliant, char::cast takes an Object as its argument causing i to be
  ...
  void quickFixes(List<String> strings) {
    ...
    strings.stream()
        .map(
            s ->
                s
                    .toLowerCase()) // Noncompliant [[sc=14;ec=16;quickfixes=qf4]] {{Replace this
