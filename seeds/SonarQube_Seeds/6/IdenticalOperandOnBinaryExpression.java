...
class IdenticalOperandOnBinaryExpressionCheck {
  void foo(boolean a, boolean b, boolean c, boolean e) {
    if (a == b) {}
    if (a
        == a) {} // Noncompliant [[sc=13;ec=14;secondary=+0]] {{Correct one of the identical
                 // sub-expressions on both sides of operator "=="}}
    if (a
        != a) {} // Noncompliant [[sc=13;ec=14]] {{Correct one of the identical sub-expressions on
  ...
  void fun(Object a, Object b) {
    a.equals(
        a); // Noncompliant {{Correct one of the identical sub-expressions on both sides of
            // equals.}}
    a.equals(b);
    equals(a);
