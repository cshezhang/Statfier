  ...
  private void f(String arg, String arg2) {
    boolean result =
        arg.toUpperCase()
            .equals(
                arg2
                    .toLowerCase()); // Noncompliant [[sc=22;ec=66]] {{Replace these
    ...
    result =
        arg.toUpperCase()
            .equals(
                arg); // Compliant - this is not case-insensitive - it checks whether arg is
                      // ALL_CAPS
    result =
        arg.toLowerCase()
    ...
    result =
        arg.toUpperCase(trLocale)
            .equals(
                BAR); // Compliant when locale is specified because it does not behave like
                      // equalsIgnoreCase
    result = FOO.equals(arg.toLowerCase(trLocale)); // Compliant

    FOO.equals(
        arg2
            .toLowerCase()); // Noncompliant {{Replace these toUpperCase()/toLowerCase() and
                             // equals() calls with a single equalsIgnoreCase() call.}}
    ...
    arg.toLowerCase()
        .toLowerCase()
        .equals(
            FOO); // Noncompliant {{Replace these toUpperCase()/toLowerCase() and equals() calls
                  // with a single equalsIgnoreCase() call.}}
    arg.toLowerCase()
        .toUpperCase()
        .equals(
            BAR); // Noncompliant {{Replace these toUpperCase()/toLowerCase() and equals() calls
                  // with a single equalsIgnoreCase() call.}}
